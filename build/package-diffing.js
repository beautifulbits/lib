...
export class PackageDiffing {
    ...
    async countDifferentFiles(packageName, remoteVersion) {
        if (!this.localLibrary)
            return null;
        const local = await this.localLibrary.grabPackageFilesAndMetadata(packageName);
        if (!local)
            return null;
        if (remoteVersion === UNPUBLISHED_VERSION) {
            return local.packageFiles.length;
        }
        const remote = await this.remoteLibrary?.grabPackageFilesAndMetadata(packageName, remoteVersion);
        if (!remote)
            return null;
        ...
        for (const rel of remoteByRel.keys()) {
            if (!seenRel.has(rel))
                count += 1; // file exists only remotely
        }
        return count;
    }
    /* ------------------------------------------------------------------------ */
    async diffWithRemotePackage(packageName, selectedRemoteVersion) {
        ...
        newRemotePackageFiles.forEach(([newFilename, newFileContents]) => {
            ...
            const newFileLinesToPrint = newFileLines.map((line, index) => `${CLI_COLORS.FgGreen}${index}. ${line}${CLI_COLORS.Reset}`);
            consola.log(boxen(subtitle + newFileLinesToPrint.join('\n'), {
    ...
    /* ------------------------------------------------------------------------ */
    printLegend(packageName, remoteVersion) {
        const lines = [
    ...
    /* ------------------------------------------------------------------------ */
    printSummary({ unchangedCount, changedCount, localOnlyCount, remoteOnlyCount, }) {
        const lines = [
    ...
    /* ------------------------------------------------------------------------ */
    printDiff({ localFileData, remoteFileData, relativeLocalFilename, remoteVersion, }) {
        if (!remoteFileData) {
            ...
            const printable = localLines.map((line, index) => `${CLI_COLORS.FgRed}${index}. ${line}${CLI_COLORS.Reset}`);
            consola.log(boxen(subtitle + printable.join('\n'), {
        ...
        let lineCount = 0;
        let diffFound = false;
        const fileContentsPrint = [];
        let prevMode = 'context';
        differences.forEach((difference) => {
            ...
            splittedSentencesWithoutExtraElement.forEach((sentence) => {
                fileContentsPrint.push(`${textColor}${lineCount}. ${sentence}${CLI_COLORS.Reset}\n`);
                ++lineCount;
            });
        });
        if (diffFound) {
