...
const BaseSurvey = (enquirer as unknown as {
  Survey: new (options: unknown) => unknown;
}).Survey as new (options: unknown) => {
  run: () => Promise<unknown>;
...
export const BUMP_TYPE_SCALE_VALUES: string[] = [
...
class BumpTypeSurvey extends BaseSurvey {
  async toChoices(...args: unknown[]): Promise<unknown> {
    // @ts-ignore
    const result = await super.toChoices(...args);
    if (result && Array.isArray(result)) {
      const defaultIdx = (this.options.defaultScaleIdx as number) ?? 0;
      for (const choice of result) {
        (choice as { scaleIdx: number }).scaleIdx = defaultIdx;
      }
    }
    return result;
  }
...
export class MainCommandsCliPrompt {
  ...
  togglePackagesPrompt({
    ...
    const checkmark = (key: string) => (selected.has(key) ? '[x]' : '[ ]');
  ...
  confirmBulkActionPrompt(message: string) {
    return new Confirm({
