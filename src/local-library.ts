...
import type {
  TReaddirFile,
  TReaddirFileExtended,
} from './@types/readdir-file.js';
import type { TPackageMetadata } from './@types/package-metadata.js';
...
interface ILocalLibraryInitFn {
  ...
  /** Optional — if provided, publish/install record into the usage registry. */
...
export class LocalLibrary {
  ...
  async findLibConfigFiles() {
    ...
    try {
      ...
      const cfgFiles = await Promise.all(
        subdirs.map(async (dir): Promise<TReaddirFile | null> => {
      ...
      this.libConfigFiles = cfgFiles.filter(
        (f): f is TReaddirFile => f !== null,
      );
  ...
  > {
    ...
    if (packageMetadata?.path && packageMetadata?.config) {
      ...
      try {
        ...
        const packageFiles: TReaddirFileExtended[] = files.map((file) => ({
          ...file,
      ...
      if (
        ...
        if (this.usageRegistry) {
          try {
            ...
            consola.warn(
              `usage registry not updated after publish: ${(err as Error).message}`,
            );
          }
        }
      }
    ...
    if (packageMetadata?.config.version === version) {
      ...
      if (this.usageRegistry) {
        try {
          ...
          consola.warn(
            `usage registry not updated after install: ${(err as Error).message}`,
          );
        }
      }
    }
